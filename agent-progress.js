...
export function startProgress(runId,goal,plan=[]){const state={runId,goal,status:'planning',currentTask:null,completedTasks:0,totalTasks:Array.isArray(plan)?plan.length:0,events:[],updatedAt:now(),clients:new Set()};runs.set(runId,state);trim();return state;}
...
function snapshot(state){const {clients,...safe}=state;return safe;}
