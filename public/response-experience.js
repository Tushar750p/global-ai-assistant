(() => {
  ...
  style.textContent = `
    ...
    .ga-response-meta{display:flex;align-items:center;gap:6px;margin-top:8px;color:#687384;font-size:9px}.ga-response-meta .ga-spacer{flex:1}
  ...
  const enhance = node => {
    ...
    if (node.classList.contains('assistant')) {
      ...
      label.className='ga-response-label';
      const dot=document.createElement('i');
      const text=document.createElement('span');
      text.textContent='Global AI';
      label.append(dot,text);
      body.insertBefore(label, bubble);
      const meta = document.createElement('div');
      meta.className='ga-response-meta';
  ...
  window.GlobalAIResponseUI = {
    activity(type,title,detail='') {
      ...
      card.className=`ga-tool-card ${type||''}`;
      const head=document.createElement('div'); head.className='ga-tool-head';
      const dot=document.createElement('i'); const titleNode=document.createElement('span'); titleNode.textContent=title;
      head.append(dot,titleNode); card.appendChild(head);
      if(detail){const body=document.createElement('div'); body.className='ga-tool-body'; body.textContent=detail; card.appendChild(body)}
