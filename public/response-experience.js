(() => {
  ...
  style.textContent = `
    ...
    .gai-tool-card{display:flex;align-items:flex-start;gap:10px;margin:10px auto;padding:10px 12px;width:min(920px,calc(100% - 24px));box-sizing:border-box;border:1px solid rgba(127,127,127,.18);border-radius:12px;background:rgba(127,127,127,.06)}
    ...
    .gai-tool-card.is-complete .gai-tool-icon::after{content:'✓';font-size:11px}.gai-tool-card.is-complete .gai-tool-icon{font-size:0}
    .gai-tool-card.is-active .gai-tool-icon{animation:gai-pulse 1.1s ease-in-out infinite}
    @keyframes gai-pulse{50%{transform:scale(.82);opacity:.55}}
  ...
  function createToolCard(type, title, detail, complete = false) {
    ...
    card.className = `gai-tool-card${complete ? ' is-complete' : ' is-active'}`;
  ...
  function enhance(message) {
    ...
    const spark = document.createElement('span'); spark.className = 'gai-spark';
    const name = document.createElement('span'); name.textContent = 'Global AI';
    label.append(spark, name); body.prepend(label);
    const meta = document.createElement('div'); meta.className = 'gai-response-meta';
    const metaText = document.createElement('span'); metaText.textContent = 'AI response';
    meta.appendChild(metaText); body.appendChild(meta);
  }
  function scan(root = document) { root.querySelectorAll?.('.message.assistant').forEach(enhance); }
  function activity(type, title, detail) {
    ...
    host.appendChild(card); host.scrollTop = host.scrollHeight; return card;
  }
  function complete(card, detail) {
    if (!card) return;
    card.classList.remove('is-active'); card.classList.add('is-complete');
    const detailEl = card.querySelector('.gai-tool-detail');
    if (detailEl && detail !== undefined) detailEl.textContent = toText(detail);
  }
  window.GlobalAIResponseUI = { activity, complete };
  scan();
