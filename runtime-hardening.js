...
express.application.post = function patchedPost(path, ...handlers) {
  if (path === '/api/chat') {
    const memoryContextMiddleware = async (req, res, next) => {
      try { const user = await getSessionUser(req); return requestContext.run({ userId: user?.id || null }, next); }
      catch { return requestContext.run({ userId: null }, next); }
    };
...
async function shutdown(signal) {
  ...
  const forceExit = setTimeout(() => { console.error('Graceful shutdown timed out; forcing process exit.'); process.exit(1); }, 10000);
  forceExit.unref();
  try { await Promise.all(findHttpServers().map(closeServer)); await closeDb(); clearTimeout(forceExit); process.exit(0); }
  catch (error) { clearTimeout(forceExit); console.error('Graceful shutdown failed:', error?.message || error); process.exit(1); }
}
