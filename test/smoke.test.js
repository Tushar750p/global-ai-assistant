...
import { hashPassword, verifyPassword, validateCredentials, sessionCookie, clearSessionCookie, createSessionToken, hashSessionToken, readSessionToken } from '../auth.js';
...
test('password verification rejects unsupported scrypt parameters and malformed hashes', async () => { const hash = await hashPassword('CorrectHorseBatteryStaple!'); const parts = hash.split('$'); parts[1] = '32768'; assert.equal(await verifyPassword('CorrectHorseBatteryStaple!', parts.join('$')), false); assert.equal(await verifyPassword('CorrectHorseBatteryStaple!', 'scrypt$16384$8$1$bad$bad'), false); assert.equal(await verifyPassword('CorrectHorseBatteryStaple!', null), false); });
test('credential validation rejects weak or malformed credentials', () => { assert.match(validateCredentials('bad-email', 'long-enough'), /valid email/); assert.match(validateCredentials('user@example.com', 'short'), /at least 8/); assert.match(validateCredentials('user@example.com', 'x'.repeat(129)), /128 characters or fewer/); assert.match(validateCredentials(`${'x'.repeat(250)}@example.com`, 'long-enough-password'), /valid email/); assert.equal(validateCredentials('user@example.com', 'long-enough-password'), null); });
test('session tokens are random, fixed-length, and hashed', () => { const a = createSessionToken(), b = createSessionToken(); assert.notEqual(a, b); assert.match(a,/^[A-Za-z0-9_-]{43}$/); assert.notEqual(hashSessionToken(a), a); assert.equal(hashSessionToken(a), hashSessionToken(a)); });
test('production cookie attributes are secure and host-only', () => { const old = process.env.NODE_ENV; process.env.NODE_ENV = 'production'; try { const cookie=sessionCookie(createSessionToken()); assert.match(cookie,/^__Host-session=/); assert.match(cookie,/HttpOnly/); assert.match(cookie,/SameSite=Lax/); assert.match(cookie,/Secure/); assert.match(cookie,/Path=\//); assert.doesNotMatch(cookie,/Domain=/); assert.match(clearSessionCookie(),/^__Host-session=/); assert.match(clearSessionCookie(),/Max-Age=0/); } finally { if (old === undefined) delete process.env.NODE_ENV; else process.env.NODE_ENV = old; } });
test('session cookie parsing rejects malformed or invalid tokens', () => { assert.equal(readSessionToken({headers:{cookie:'session=abc'}}), null); assert.equal(readSessionToken({headers:{cookie:'__Host-session=abc'}}), null); const token=createSessionToken(); assert.equal(readSessionToken({headers:{cookie:`__Host-session=${token}`}}), token); });
